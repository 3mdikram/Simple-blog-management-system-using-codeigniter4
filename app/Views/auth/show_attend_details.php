            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Attendence Details</h3>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
                            ...
                            <tbody>
                                <?php if (count($results)) {
                                    foreach ($results as $result) {
                                        $stime = strtotime($result['work_stime']);
                                        $endtime = strtotime($result['work_endtime']);
                                        $hour = round(abs($endtime - $stime)/(60*60),2);
                                        //$fullName = $fname . " " . $lname;
                                ?>
                                        <tr>
                                            <td><?php echo date("g:i A",strtotime($result['work_stime'])); ?></td>
                                            <td><?php echo date("g:i A",strtotime($result['work_endtime'])); ?></td>
                                            <td><?php echo $hour." Hours";?></td>
                                            <td class="text-center">
                                            <?php echo date("D jS M Y ",strtotime($result['current_date'])); ?>
                                            </td>
                                        </tr>
                                <?php }
                                } else {
